...
using Hire.Services.Extensions;
...
namespace Hire.WebApi.Filters
{
    public class LinkRewritingFilter : IAsyncResultFilter
    {
        ...
        public Task OnResultExecutionAsync(ResultExecutingContext context, ResultExecutionDelegate next)
        {
            ...
            var shouldSkip = asObjResult?.StatusCode >= 400 || !(asObjResult?.Value is Resource _);
        ...
        private static void RewriteAllLinks(object model, LinkRewriter rewriter)
        {
            ...
            var linkProperties = allProperties
                .Where(p => p.CanWrite && p.PropertyType == typeof(Link)).ToList();
